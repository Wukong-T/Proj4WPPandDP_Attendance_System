<template>
 ...
 <div>
        ...
        <el-row>
          ...
          <el-form
            ...
            label-width="100px">
                ...
                <el-form-item label="所在组: " v-show="selectTeacher">
                  <div style="text-align:left;" >
                ...
                <el-form-item label="组号" prop="classes" v-show="selectClass" >
                  <el-input
                    placeholder="请输入组号"
                ...
                <el-form-item label="部门 " prop="dept" v-show="selectDept">
                  <el-input
                    placeholder="请输入部门"
                ...
                <el-form-item label="选择用户 " prop="choice" >
                  <el-input
                  disabled
                    placeholder="请输入选择用户"
          ...
          <el-form
            ...
            label-width="100px">
                ...
                <el-form-item  label="选择用户" prop="choice" style="margin-left:5px; text-align: center" >
                  <el-select 
                    @change="Hide(insertForm.choice)"
                    v-model="insertForm.choice" 
                    placeholder="请选择用户类型"
                    prop="choice">
                ...
                <el-form-item label="组号 " prop="classes" v-show="selectClass">
                  <el-input
                    placeholder="请输入组号"
                    prefix-icon="el-icon-school"
                    v-model.trim="insertForm.classes"
                  ></el-input>
                </el-form-item>
    ...
    <div>
      ...
      <el-row >
        <el-col :span="17" :xs="17" :sm="17" :md="17" :lg="17" :xl="17">
      ...
      <el-table-column prop="classes" label="班级" align="center" width="100px" v-if="selectTableClass"></el-table-column>
      <el-table-column prop="dept" label="院系" align="center" width="150px" v-if="selectTableDept"></el-table-column>
...
export default {
  ...
  data(){
    var validateClass = (rule,value,callback) => {
        if (this.ruleForm.choice == "员工" 
          || this.insertForm.choice == "员工") {
          if (!value) {
          return callback(new Error("组号不能为空"));
          } else {
          callback();
        }
        }else {
    ...
    var validateDept = (rule,value,callback) => {
        if (this.ruleForm.choice == "副经理"
          ||this.ruleForm.choice == "员工"
          || this.ruleForm.choice == "部门经理"
          || this.insertForm.choice == "副经理"
          ||this.insertForm.choice  == "员工"
          ||this.insertForm.choice == "部门经理") { 
          if (!value) {
  ...
  methods:{
        ...
        switch(resp.data){
          case "empty":
            _this.$message.error("辅导员姓名不能为空！");
              _this.resetForm('insertForm')
            break;
          case "repeat":
            _this.$message.error("该组已有人管理!不可重复添加！");
             window.location.reload();  
          break;
    ...
    findAll(){
       var _this=this;
      axios.get('http://localhost:8181/data/findAll/'+_this.currentPage+'/'+_this.pageSize).then(function(resp){
    ...
    showByChoice(){
     var formName = this.searchForm.choice;
      switch(formName){
        case "全部":
          this.selectTableClass = false;
          this.selectTableDept  = false;
          this.findAll();
          break;  
        case "学生":
          this.findAllByChoice(formName);
          this.selectTableClass = true;
          this.selectTableDept  = true;
          break;
      case "教务处":
          this.findAllByChoice(formName);
          this.selectTableClass = false;
   ...
   this.showByChoice();
    ...
    //删除全部
    delAll() {
      this.$confirm("此操作将永久删除所有用户信息, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          const _this = this;
          axios
            .delete("http://localhost:8181/data/deleteData/")
            .then(function (resp) {
              _this.$alert(" 所有用户基本信息均已删除！", "消息", {
                confirmButtonTest: "确定",
                callback: (action) => {
                  window.location.reload();}
              });
    ...
    exportExcel() {
      require.ensure([], () => {
        ...
        const tHeader = ['真实姓名', '性别', '年龄','学号/职工号', '班级', '院系','用户类型'];
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['realname', 'sex', 'age','username', 'classes', 'dept','choice'];
        // 上面的index、nickName、name是tableData里对象的属性
        const list = this.tableData;  //把data里的tableData存到list
        const data = this.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, '用户信息表');//"列表excel"  是下载后的表名 可修改
      })
    },
  ...
  delFile() {
	...
	handleChange(file, fileList) {
      ...
      //  console.log(file.name)
       if(file.name === "用户信息表.xlsx"||file.name === "用户信息表.xls"){
         	this.fileList = fileList;
       }else{
          this.$message.error("只能上传用户信息表！");
         this.delFile()
       }
	},
	//单击上传到服务器（提交）按钮
