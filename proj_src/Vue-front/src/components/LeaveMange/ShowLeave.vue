    ...
    <el-card>
      <el-col :span="24" :xs="24" :sm="24" :md="24" :lg="24" :xl="24">   
        ...
        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
          <el-form-item >
            <el-tag effect="dark" type="info" style="font-size:15px; margin:5px;text-align: left;">
              <h2>以往审核记录</h2>
            </el-tag>
          </el-form-item>
           

          <el-form-item style="margin:5px"  label="按审核状态显示" prop="choice" >
                  <el-select 
                    ...
                    <el-option label="副经理待审" value="副经理待审" v-show="selectAuditTeacher"></el-option>
                    <el-option label="总经理待审" value="总经理待审" v-show="selectAuditLeader"></el-option>
      ...
      <div style="text-align: left; ">
        ...
        <el-form :inline="true"  class="demo-form-inline">
          ...
          <div style="text-align:left;font-size:12px; margin:24px;margin-bottom:20px;">
            <el-tag  effect="plain" type="danger" style="height:100%;width:100%text-align: left;" > 
          <i style="text-align: left;font-size:12px;color:#000;">
            1、如果员工再次提交申请表，则无法重新审理。<br>
            2、如果对审批的结果操作失误，可进行重新审理；<br>
            3、非最终审批人审核通过后传给下位，无记录。审核不通过可查可重审。<br>
            4、最终审批人由申请表的时间决定，三天以内为部门经理；
            三天以上一周以内为副经理；一周以上四周以内由总经理管理。
            </i> </el-tag>
          </div>
          </el-form-item>
  ...
  methods:{
    ...
    findAllLeave(){
      ...
      _this.username = window.sessionStorage.getItem("username");
        axios.get('http://localhost:8181/leave/ShowLeave/'+_this.currentPage+'/'+_this.pageSize+'/'+_this.username+"/"+_this.searchForm.state).then(function(resp){
    ...
    examLeave(formName){
        ...
        .then(function (resp) {
            ...
            case "repeat":
                _this.$message.error("该员工已经重新提交新的申请表！无需重审！");
