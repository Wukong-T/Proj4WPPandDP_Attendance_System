<template>
        ...
        :close-on-click-modal="false"
          ...
          <el-form
            ...
            label-width="100px">
                ...
                <el-form-item label="组号 " prop="classes">
                  <el-input
                    placeholder="请输入组号"
 ...
 <el-row>
    ...
    <el-table class="mytable" :data="tableData" border style="width: 850px">
   ...
   <el-col :span="4" :xs="4" :sm="4" :md="4" :lg="4" :xl="4">
      ...
      accept=".xls,.xlsx">
        <el-button size="small"  slot="trigger" type="primary" @click="delFile">选取文件</el-button>
      ...
      <el-button size="small" style="margin-left: 10px;margin:30PX"  type="danger" 
            @click="delAll()" >删除全部员工信息</el-button>
...
export default {
  ...
  data(){
    ...
    if (this.ruleForm.choice == "部门经理"||this.ruleForm.choice == "员工") {
      if (!value) {
      return callback(new Error("请输入组号"));
      } else {
    callback();
    }
    }else {
    ...
    var validateDept = (rule,value,callback) => {
        if (this.ruleForm.choice == "副经理"||this.ruleForm.choice == "员工") {
          if (value === "") {
    ...
    return{
      ...
      rules: {
        username: [
          ...
          { min: 10, message: "请输入正确的工号", trigger: "blur" },
          { max: 10, message: "请输入正确的工号", trigger: "blur" },
          {
  ...
  methods:{
    ...
    delAll() {
      this.$confirm("此操作将永久删除所有员工信息, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          axios
            .delete("http://localhost:8181/data/deleteDataByChoice/学生")
            .then(function (resp) {
              _this.$alert(" 所有员工的基本信息均已删除！", "消息", {
	...
	handleChange(file, fileList) {
	      if(file.name === "学生信息表.xlsx"||file.name === "学生信息表.xls"){
         	this.fileList = fileList;
       }else{
          this.$message.error("只能上传学生信息表！");
         this.delFile()
       }
	},
	//单击上传到服务器（提交）按钮
     ...
     switch(resp.data){
      ...
      exportExcel() {
        require.ensure([], () => {
          ...
          const tHeader = ['真实姓名', '性别', '年龄','工号', '组号', '部门','用户类型'];
          // 上面设置Excel的表格第一行的标题
          const filterVal = ['realname', 'sex', 'age','username', 'classes', 'dept','choice'];
          // 上面的index、nickName、name是tableData里对象的属性
          const list = this.tableData;  //把data里的tableData存到list
          const data = this.formatJson(filterVal, list);
          export_json_to_excel(tHeader, data, '学生信息表');//"列表excel"  是下载后的表名 可修改
        })
      },
