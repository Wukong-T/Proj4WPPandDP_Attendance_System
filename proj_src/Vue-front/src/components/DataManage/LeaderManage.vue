<template>
 <body>  
    ...
    <div>
      ...
      <el-row>
        <el-col :span="16">
    <el-table class="mytable" :data="tableData" border  type="expand"  style="width: 100%">
      ...
      <el-table-column prop="sex"  label="性别" align="center" width="50px"></el-table-column>
       ...
       <el-table-column prop="username" label="职工号" align="center" width="100px"></el-table-column>
      <!-- <el-table-column prop="classes" label="组号"  width="100px"></el-table-column> -->
      ...
      accept=".xls,.xlsx">
        ...
        <div slot="tip" style="margin-left: 10px;" class="el-upload__tip">只能上传xls/xlsx文件</div>
          ...
          <el-button size="small" style="margin-left: 10px;margin:30PX"  type="danger" 
            @click="delAll()" >删除全部副经理信息</el-button>
...
export default {
  ...
  methods:{
    ...
    delAll() {
      this.$confirm("此操作将永久删除所有副经理信息, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          axios
            .delete("http://localhost:8181/data/deleteDataByChoice/院领导")
            .then(function (resp) {
              _this.$alert(" 所有副经理的基本信息均已删除！", "消息", {
 ...
 delFile() {
	...
	handleChange(file, fileList) {
      if(file.name === "副经理信息表.xlsx"||file.name === "副经理信息表.xls"){
         	this.fileList = fileList;
       }else{
          this.$message.error("只能上传副经理信息表！"); 
         this.delFile()
         }
	},
	//单击上传到服务器（提交）按钮
 ...
 exportExcel() {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['真实姓名', '性别', '年龄','职工号', '组号', '部门','用户类型'];
        // 上面设置Excel的表格第一行的标题
        const filterVal = ['realname', 'sex', 'age','username', 'classes', 'dept','choice'];
        // 上面的index、nickName、name是tableData里对象的属性
        const list = this.tableData;  //把data里的tableData存到list
        const data = this.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, '院领导信息表');//"列表excel"  是下载后的表名 可修改
      })
    },
